  ...
  return (
    <section className="flex flex-wrap flex-col max-w-full h-auto bg-[#373747] items-center p-0 m-0 text-white">
        ...
        <div className="flex flex-wrap flex-row">
            ...
            <table className="text-xs">
              ...
              <tbody>
                <TableData
                  icon={Bomb}
                  type="BOMB"
                  currSupply={bombCirculatingSupply}
                  totalSupply={bombTotalSupply}
                  priceinDollar={bombPriceInDollars}
                ...
                <TableData
                  icon={BShares}
                  type="BSHARE"
                  currSupply={bShareCirculatingSupply}
                  totalSupply={bShareTotalSupply}
                  priceinDollar={bSharePriceInDollars}
                ...
                <TableData
                  icon={BBond}
                  type="BBOND"
                  currSupply={tBondCirculatingSupply}
                  totalSupply={tBondTotalSupply}
                  priceinDollar={tBondPriceInDollars}
          ...
          <div className="flex flex-wrap flex-col w-1/3 items-center justify-end">
            <div className="flex flex-wrap h-20 justify-center items-center border-b-2 border-b-[#C3C5CBBF]">
              <span>
                Current Epoch
                <p className="flex justify-center items-center text-2xl font-semibold">{Number(currentEpoch)}</p>
              </span>
            </div>
            <div className="flex flex-wrap h-20 justify-center items-center border-b-2 border-b-[#C3C5CBBF]">
              <span>
            ...
            <div className="flex flex-wrap h-20 justify-center items-center text-[#C3C5CBBF]">
              <span>
                <h4 className="flex justify-center items-center">
                  Live TWAP:&nbsp;<p className="text-[#00E8A2]">{scalingFactor}</p>
                </h4>
                <h4 className="flex justify-center items-center">
                  TVL:&nbsp;
                  <p className="text-[#00E8A2]">
                    <CountUp end={TVL} separator="," prefix="$" />
                  </p>
                </h4>
                <h4 className="flex justify-center items-center">
                  Last Epoch TWAP:&nbsp;<p className="text-[#00E8A2]">{bondScale || '-'}</p>
                </h4>
              </span>
            </div>
          </div>
      ...
      <div className="flex flex-wrap flex-row h-96 w-3/4 m-2">
        <div className="flex flex-wrap flex-col w-2/3 p-2">
          <div className="flex flex-wrap w-full justify-end">
            <a
              ...
              className="underline text-[#9EE6FF] opacity-75"
            >
          ...
          <button
            onClick={() => {
              window.open('https://app.bogged.finance/bsc/swap', '_blank');
            }}
            className="bg-[#00ADE8] opacity-75 font-bold p-1 m-1 border-[#E41A1A] border-2"
          >
            Invest Now
          </button>
          <div className="flex flex-wrap flex-row h-11">
            <button
              onClick={() => {
                window.open('https://discord.com/invite/94Aa4wSz3e', '_blank');
              }}
              className="bg-slate-400 opacity-75 text-black h-9 border-[#728CDF] border-2 w-[299px] justify-center p-1 m-1"
            >
              <span className="inline-flex">
                <img src={Discord} alt="" className="h-6 w-6 p-1 rounded-full bg-white" />
                &nbsp;Chat on Discord
              </span>
            </button>
            <button
              onClick={() => {
                window.open('https://docs.bomb.money/welcome-start-here/readme', '_blank');
              }}
              className="bg-slate-400 opacity-75 text-black h-9 border-[#728CDF] border-2 w-[300px] justify-center p-1 m-1"
            >
              <span className="inline-flex">
                <img src={Docs} alt="" className="h-6 w-6 p-1 rounded-full bg-white" />
                &nbsp;Read Docs
              </span>
            </button>
          </div>
        ...
        <div className="flex flex-wrap bg w-1/3 h-96 p-3 rounded-lg border-[#728CDF] border-2 text-lg font-semibold">
          Latest News
        </div>
      </div>
      ...
      <div className="flex flex-wrap flex-col h-[700px] w-3/4 bg p-3 rounded-lg border-2 border-[#728CDF]">
        <div className="flex flex-wrap flex-row h-20">
          ...
          <div className="flex flex-wrap w-[50%] justify-end">
            ...
            >
              <span className="inline-flex">
                Claim All&nbsp;
                <img src={BShares} alt="" className="h-6 w-6 p-1 bg-gray-700 rounded-full" />
              </span>
            </button>
          </div>
        </div>
        ...
        {activeBanks.map((bank) => {
          return (
            <AccessOnebyOne
              key={bank.poolId}
              type={bank.depositTokenName.replace('-LP', '')}
              bankId={bank.contract}
              stakeIcon={bank.poolId === 1 ? BombBitcoin : BombbnbLP}
              earnedIcon={BShares}
            />
          );
        })}
          ...
          <div className="flex flex-wrap flex-col w-2/4 p-3">
              ...
              <div className="flex flex-wrap justify-end w-2/4">
                <button
                  disabled={
                    ...
                    (approveStatus === ApprovalState.UNKNOWN && !bondStat) ||
                    isBondRedeemable
                  }
                  onClick={() => catchError(approve(), `Unable to approve BOMB`)}
                  className={`border-2 rounded-full h-10 w-36 pt-1 pb-1 ${
                    approveStatus === ApprovalState.PENDING ||
                    (approveStatus === ApprovalState.UNKNOWN && !bondStat) ||
                    isBondRedeemable
                      ? 'border-[#C3C5CBBF] text-[#C3C5CBBF]'
            ...
            <div className="flex flex-wrap h-20 mt-5 p-2">
              ...
              <div className="flex flex-wrap justify-end w-2/4">
                <button
                  disabled={
                    redeemStatus === ApprovalState.PENDING ||
                    (redeemStatus === ApprovalState.UNKNOWN && !bondStat) ||
                    bondBalance.eq(0) ||
                    !isBondRedeemable
                  }
                  onClick={() => catchError(redeem(), `Unable to redeem BBOND`)}
                  className={`border-2 rounded-full h-10 w-36 pt-1 pb-1 ${
                    approveStatus === ApprovalState.PENDING ||
                    (approveStatus === ApprovalState.UNKNOWN && !bondStat) ||
                    bondBalance.eq(0) ||
